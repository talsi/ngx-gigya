...
export class ZoneAwareProxyService {
  ...
  public run(fn: (...data: any[]) => void, fnName: string, ...params: any[]): Promise<any> {
    const asyncResult = this.wrapAllCallbacksToRunInsideNgZone(fn, fnName, params);
  ...
  private wrapAllCallbacksToRunInsideNgZone(fn: any, fnName: any, params: any[]): Promise<any> {
    ...
    params.forEach(param => {
      ...
      if (typeof param === 'function') {
        mainPromise = new Promise(resolve => {
          param = this.wrapSingleCallbackToRunInsideNgZone(fnName, param, resolve);
        });
      }
        ...
        Object.keys(param).forEach(key => {
          if (typeof param[key] === 'function') {
            ...
            const promise = new Promise(resolve => {
              param[key] = this.wrapSingleCallbackToRunInsideNgZone(fnName, param[key], resolve, key);
            });
    ...
    if (!mainPromise) {
      params.forEach(param => {
        if (typeof param === 'object') {
          mainPromise = new Promise(resolve => {
            param.callback = this.wrapSingleCallbackToRunInsideNgZone(fnName, noop, resolve);
          });
        }
      });
    }
  ...
  // @ts-ignore
  private wrapSingleCallbackToRunInsideNgZone(fnName: string, callbackFunc, resolve, callbackName?: string): (...args: any[]) => void {

    if (!callbackName) {
      callbackName = callbackFunc.name || 'anonymous';
    }

    ZoneAwareProxyService.log(`wrapping ${fnName}'s callback "${callbackName}" to run inside angular zone`);

    return (...args: any[]) => {
      ZoneAwareProxyService.log(`running ${fnName}'s callback "${callbackName}" inside angular zone`, args);
